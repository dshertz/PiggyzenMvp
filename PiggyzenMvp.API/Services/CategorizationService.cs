...
namespace PiggyzenMvp.API.Services
{
    public class CategorizationService
    {
        ...
        )
        {
            ...
            // Hitta existerande regel för denna payee/tecken/kategori (beloppsintervall valfritt i MVP)
            var history = await _context.CategorizationRules.FirstOrDefaultAsync(
                h =>
            ...
            if (history == null)
            {
                ...
                _context.CategorizationRules.Add(history);
            }
            ...
            {
                // Om byter regel: markera ev. override på tidigare val (spårbarhet)
                if (u.CategorizationRuleId != history.Id)
                {
                    u.WasOverridden = DateTime.UtcNow;
                    u.CategorizationRule = history;
        ...
        private static bool InRange(CategorizationRule h, decimal amount)
        {
